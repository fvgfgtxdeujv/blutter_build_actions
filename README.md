...
**并行机制**：先由 `prepare` job 解析版本列表并计算 worker 数（版本数少于 20 时每版本一个 worker，最多 20 个），再按列表顺序轮流分片给各 worker，每个 worker 一个并行 job。例如 10 个版本在 22.04 下就是 10 个 job 并行编译（`build-dart-version` 界面中「构建 Worker 1~10」）。

**增量补齐逻辑**：每个 worker 处理自己分片内的版本时，先检查该版本 Release 是否已有本环境后缀的产物——已有则跳过该版本（不构建不发布）；没有则构建并发布。例如选择 `24.04` 时，已有 `_22` 的版本会补上 `_24`，已有 `_24` 的版本会跳过；选择 `22.04` 时对称。单个版本构建失败自动跳过，不影响其他版本。
