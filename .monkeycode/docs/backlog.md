...
分支基线：`master` `77f894c`（A/B 见 `3596f9e`，C/D 见 `36b1217`，DartTypes/格式化修复见 `84d135a`，E 见 `92e71f8`）
...
- 状态：**已落地**（2026-09-21，提交 `92e71f8`）。实现：`ILInstr` 新增 `virtual intptr_t PoolOffset()`（默认 -1），`StoreObjectPoolInstr` 返回其池偏移，`InitLateStaticFieldInstr` 新增构造参数 `poolOffset`（arm64 由 `objPoolInstr.item.storage.offset` 传入，默认 -1 兼容其他调用点）；`DumpCode` 打印 IL 行时若 `PoolOffset() >= 0` 则追加 `  ; ` + `getPoolObjectDescription(offset)`，与 asm 行的池描述一致
